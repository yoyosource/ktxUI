...
fun <V: ViewContainer> V.Background(color: Color, builder: DrawableSingleViewContainer.() -> Unit) {
    (+Background(ViewOption(color))).builder()
}

fun <V: ViewContainer> V.Background(color: KProperty0<Color>, builder: DrawableSingleViewContainer.() -> Unit) {
    val _color = ViewOption(Color.BLACK)
    (+Background(_color)).apply(builder).let {
        _color.set(it, color)
    }
}
...
fun <V: View> V.background(color: Color): V {
    val background = Background(ViewOption(color))
    parent!!.swap(this, background)
    return this
}

fun <V: View> V.background(color: KProperty0<Color>): V {
    val _color = ViewOption(Color.BLACK)
